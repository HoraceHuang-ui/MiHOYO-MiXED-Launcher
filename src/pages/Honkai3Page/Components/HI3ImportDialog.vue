...
const hiLauncherImport = async () => {
    window.dialog.show({
        title: translate('general_launcherImportTitle', {game: translate('general_hi3')}),
...
const hiGameImport = async () => {
    window.dialog.show({
        title: translate('general_gameImportTitle', {game: translate('general_hi3')}),
        properties: ['openFile'],
...
<template>
    <Honkai3Dialog :title="`${translate('general_hi3')} ${translate('general_import')}`" width="50%" :on-ok="onDialogOk"
                   :on-cancel="onCancel"
                   show-ok show-cancel :close-on-ok="false"
                   ref="dialogRef">
        <div class="px-1 grid grid-cols-2 gap-4">
            <button @click="hiLauncherImport"
            ...
            <div class="ml-3 text-gray-200" style="margin-top: 5px; grid-column: 1 / 3;">
                <span class="theme-color font-bold mr-2">{{ translate('general_launcher') }}</span>
            ...
            <div class="flex flex-row">
                <el-checkbox v-model="combinePaths" class="text-gray-200">{{
                        translate("general_defaultStructure")
                    }}
                </el-checkbox>
